...
export function resolve_csv_path(file_path: string): string {
...
export function validate_headers(headers: string[]): void {
	if (headers.length === 0) {
...
export function infer_sqlite_type(
	values: unknown[],
): 'INTEGER' | 'REAL' | 'TEXT' {
...
export function get_table_columns(
...
export function table_exists(database_path: string, table: string): boolean {
	const db = open_database(database_path);
	const row = db
		.prepare(
...
export function create_table_from_csv(
	database_path: string,
	table: string,
	headers: string[],
...
export function validate_import_columns(
	csv_headers: string[],
...
export function row_error_message(error: unknown): string {
